...
from bbn_medic.detection.agentic.hybrid.mistral import AgenticDetector
...
def client_main(input_questions_jsonl, input_answers_jsonl, focus_task, agent_group_mode, model_path, model_api_url,
    ...
    num_answers = 0
    for line in JSONLGenerator.read(input_answers_jsonl):
        num_answers += 1
    outputs = []
    for index, line in tqdm(enumerate(JSONLGenerator.read(input_answers_jsonl)), desc="Processing JSONL file",
                            unit="lines", total=num_answers):
        obj = line
        answer_id = obj.id
        ...
        # New processing:
        if len(original_answer.split(" ")) > 200:
            original_answer = " ".join(original_answer.split(" ")[:200])
            logging.info(f"Truncated line {index} to length 200: \n{original_answer}\n\n")
...
def debug_main():
    ...
    client_main(input_questions_jsonl, input_answers_jsonl, focus_task, agent_group_mode, model_path, model_api_url,
                temperature,
                output_path)
