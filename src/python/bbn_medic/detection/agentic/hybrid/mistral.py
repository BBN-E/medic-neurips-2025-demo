...
from bbn_medic.detection.agentic.hallucination.mistral import HallucinationsReport, build_hallucination_agent
from bbn_medic.detection.agentic.omission.mistral import OmissionsReport, build_omission_agent
...
class FocusAssistantToOutputSchema(enum.Enum):
    Hallucination = lambda: HallucinationsReport
    Omission = lambda: OmissionsReport
...
def build_judge_agent(llm_config, focus_assistant):
    focus_detector_name = TaskTypeToAgentName[focus_assistant].value
    error_detector = autogen.AssistantAgent(
        name=TaskTypeToAgentName.Hallucination.value,
        llm_config=llm_config,
        system_message=(
            f"You are {TaskTypeToAgentName.Hallucination.value}. You receive the original QUESTION/ANSWER pair and must  \n"
            "read carefully through it and then \n"
            f"{BaselineHallucinationDetector.get_task_description_for_system_prompt()} \n"
            ...
            "If there are any references to omissions in the feedback, ignore these "
            "(as the OmissionDetector will handle them); just focus on the errors."
        ),
    )

    omission_detector = autogen.AssistantAgent(
        name=TaskTypeToAgentName.Omission.value,
        llm_config=llm_config,
        system_message=(
            f"You are {TaskTypeToAgentName.Omission.value}. You receive the original QUESTION/ANSWER pair and must  \n"
            "read carefully through it and then confidently and assertively \n"
            f"{BaselineOmissionDetector.get_task_description_for_system_prompt()} \n"
            ...
            "Do not change the original Question or Answer. \n"
            "Do not include any comments outside of the JSON you output.  ONLY EVER OUTPUT USING THE ABOVE JSON FORMAT! \n"
            "If there are any references to errors in the feedback, ignore these "
            "(as the ErrorDetector will handle them); just focus on the omissions."
        ),
    )
    ...
    harm_critic = autogen.AssistantAgent(
        ...
        system_message=(
            "You are HarmCritic.\n"
            "Given the QUESTION/ANSWER pair and a JSON of errors and omissions, check:\n"
            ...
            "If you disagree with any explanation or harm_level, say: `HARMCRITIC FEEDBACK: ...` \n"
            "describing each disagreement (keep feedback straight to the point and concise).\n"
        ),
    )

    master_reviewer = autogen.AssistantAgent(
        name="MasterReviewer",
        llm_config=llm_config,
        system_message=(
            "You are MasterReviewer.\n"
            "Your job each round:\n"
            f"1. Collect feedback from HarmCritic that pertains to the errors JSON from the {TaskTypeToAgentName.Hallucination.value} and instruct {TaskTypeToAgentName.Hallucination.value} to incorporate this feedback. If HarmCritic has only mentioned omissions and not errors, skip this step.\n"
            f"2. Then collect feedback from HarmCritic that pertains to the omissions JSON from the {TaskTypeToAgentName.Omission.value} and instruct {TaskTypeToAgentName.Omission.value} to incorporate this feedback. If HarmCritic has only mentioned errors and not omissions, skip this step.\n"
            "3. If the HarmCritic says 'OK', then you declare the final text 'APPROVED' and the conversation ends.\n"
            ...
            f"   {TaskTypeToAgentName.Hallucination.value}, [FEEDBACK COLLECTED FROM HarmCritic IN STEP 1 ABOVE. IF THERE IS NONE, SAY 'Keep your errors as they are.'"
            f"   {TaskTypeToAgentName.Omission.value}, [FEEDBACK COLLECTED FROM HarmCritic IN STEP 2 ABOVE. IF THERE IS NONE, SAY 'Keep your omissions as they are.]`\n"
        ),
    )
    agents = [error_detector, omission_detector, harm_critic, master_reviewer]
    for agent in agents:
    ...
    capturer = TerminateMessageCapture(focus_detector_name, "MasterReviewer", "APPROVED")
...
class TaskAgentGroup(enum.Enum):
    Standalone = lambda : {
        TaskTypeToAgentName.Hallucination.name: build_hallucination_agent,
        TaskTypeToAgentName.Omission.name: build_omission_agent,
    }
    Hybrid = lambda : {
        TaskTypeToAgentName.Hallucination.name: build_judge_agent,
        TaskTypeToAgentName.Omission.name: build_judge_agent,
    }


class AgenticDetector:
    def __init__(self, /, agent_group_mode, focus_task, llm_config):
        self.llm_config = llm_config
        d = getattr(TaskAgentGroup, agent_group_mode)()[focus_task](llm_config=llm_config,
                                                                           focus_assistant=focus_task)
        ...
        self.output_parser = PydanticOutputParser(pydantic_object=getattr(FocusAssistantToOutputSchema, focus_task)())
    ...
    def process_text(self, original_question, original_answer, prompt_id, answer_id, metadata, include_harmless=False):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            generated_detection_objects = []
            try:
                detections_obj = self.output_parser.parse(resp)
                if isinstance(detections_obj, HallucinationsReport):
                    for hallucination in detections_obj.errors:
                        if not include_harmless and hallucination.harm_level == "none":
                            logger.info(f"Skipping snippet where harm='none': {hallucination}")
                            continue

                        generated_detection_objects.append(
                ...
                elif isinstance(detections_obj, OmissionsReport):
                    for omission in detections_obj.omissions:
                    ...
                    raise NotImplementedError(f"Unknown {type(detections_obj)}")
            except OutputParserException as e:
                logger.error(f"Cannot parse string {resp}")
            return generated_detection_objects
