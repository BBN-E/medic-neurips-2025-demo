...
logger = logging.getLogger("QABaselineHallucinationDetector.py")


class QABaselineHallucinationDetector:
    ...
    def detect(prompts_input_file,
        ...
        #      and {QABaselineHallucinationDetector.get_task_description_for_system_prompt()}""",
        #     input_variables=[],
        #     partial_variables={}
        # )
        system_prompt = f"""You are a medical expert. Please read carefully through the following QUESTION/ANSWER pair
             and {QABaselineHallucinationDetector.get_task_description_for_system_prompt()}"""
        ...
        with fopen(output_file, "w") as g:
            for obj in tqdm(JSONLGenerator.read_all(answers_input_file), file=sys.stdout,
                ...
                # Model does not support Structured Outputs, so add the instructions into the human message:

                messages = [HumanMessage(content=user_prompt_template.format(question=prompt_ids_to_prompts[prompt_id].text,
                                            answer=answer_text, output_instructions=output_instructions if output_instructions else ""))]
                    ...
                    try:
                        ...
                        for hallucination in detections_obj.detections:
                            ...
                            generated_hallucination_objects.append(
                                Hallucination(detector_type="QABaselineHallucinationDetector",
