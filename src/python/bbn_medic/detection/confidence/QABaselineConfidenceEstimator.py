...
class BaselineConfidenceEstimator:
    ...
    def detect(prompts_input_file, answers_input_file, output_file, model_instance, model_temp, retry_max_attempts=1):
        ...
        system_prompt = "You are a medical expert. Read carefully the following QUESTION and ANSWER pair and determine the level of confidence indicated by the language of the ANSWER."
        user_prompt_template = PromptTemplate(
            template=system_prompt+"""QUESTION: {question}\n ANSWER: {answer}\n{output_instructions}""",
        ...
        with fopen(output_file, "w") as g:
            for obj in JSONLGenerator.read(answers_input_file):
                ...
                messages = [user_prompt_template.format(question=prompt_ids_to_prompts[prompt_id].text,
                                                        answer=answer_text, output_instructions = output_instructions)]
